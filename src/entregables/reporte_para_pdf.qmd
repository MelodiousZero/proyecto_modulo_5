...
\hfill
\begin{minipage}[t]{0.45\textwidth}
Separamos la limpieza del dataset y el modelaje en dos \texttt{.py} usando programación orientada a objetos para que después sean llamados dentro de los \texttt{.ipynb} usando sus respectivos métodos de clase.
\end{minipage}
...
![profile report](dataset_statistics.png){width=50%}
...
for i, col in enumerate(categorical_columns):
    ...
    axes[i].set_ylabel('Rented Bike Count', fontsize=6)
    axes[i].tick_params(axis='x', rotation=45, labelsize=3)
...
# Definir modelos y sus nombres
modelos = [
    ('Lasso', lasso.coef_),
    ('Ridge', ridge.coef_),
    ('Regresión Lineal', linear_regressor.coef_),
...
for ax, (nombre, coefs) in zip(axes, modelos):
    ax.bar(features, coefs)
    ax.set_xticklabels(features, rotation=90, fontsize=4)
    ax.set_title(f'Coeficientes – {nombre}', fontsize=6)
